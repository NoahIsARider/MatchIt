// maximum_calculator.js
// JavaScript for calculating field maximum values in form submissions

document.addEventListener('DOMContentLoaded', function() {
    // Get elements
    const fieldSelect = document.getElementById('field-select');
    const calculateBtn = document.getElementById('calculate-maximum-btn');
    ...
    calculateBtn.addEventListener('click', function() {
        ...
        // Make AJAX request to calculate maximum
        fetch(`/api/calculate_maximum/${formId}/${fieldName}`)
            .then(response => {
                if (!response.ok) {
            ...
            .then(data => {
                ...
                if (data.maximum !== null) {
                    // Format the maximum to 2 decimal places
                    const formattedMaximum = parseFloat(data.maximum).toFixed(2);
                    
                    averageValueElement.innerHTML = `<strong>Maximum value for ${fieldName}:</strong> ${formattedMaximum}`;
            ...
            .catch(error => {
                console.error('Error calculating maximum:', error);
                averageValueElement.innerHTML = '<strong>Error calculating maximum</strong>';
            ...
            .finally(() => {
                ...
                calculateBtn.innerHTML = 'Calculate Maximum';
            });
    });
});